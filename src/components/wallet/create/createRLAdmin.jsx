...
import { AlertDialog } from '@venidium/core';
import {
  Typography,
  Button,
  Box,
...
import { venidium_to_ray } from '../../../util/venidium';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      venidiumper_input.value === '' ||
      Number(venidiumper_input.value) === 0 ||
      !Number(venidiumper_input.value) ||
      isNaN(Number(venidiumper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const venidiumper = venidium_to_ray(venidiumper_input.value);
    const venidiumper_value = Number.parseInt(Number(venidiumper));
    const userpubkey = userpubkey_input.value;
    const amount = venidium_to_ray(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = venidium_to_ray(fee_input.value);

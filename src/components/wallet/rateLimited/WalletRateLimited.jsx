...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const venidiumper_input = ip_parsed.limit;
    ...
    const venidiumper_value = Number.parseInt(Number(venidiumper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (venidium per interval):{' '}
                  {ray_to_venidium_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = venidium_to_ray(fee_input.value);

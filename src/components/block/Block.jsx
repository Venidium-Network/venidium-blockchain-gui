...
export default function Block() {
  ...
  const poolReward = ray_to_venidium(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = ray_to_venidium(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const venidiumFees = blockRecord.fees
    ? ray_to_venidium(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.venidiumexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.venidiumexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: venidiumFees ? `${venidiumFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Venidium blockchain
            </Trans>
          </BlockTitle>
        }
        action={

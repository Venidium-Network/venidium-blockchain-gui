...
import venidiumEnvironment from '../util/venidiumEnvironment';
import venidiumConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!venidiumEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    venidiumConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!venidiumConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !venidiumConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (venidiumConfig.manageDaemonLifetime()) {
        venidiumEnvironment.startVenidiumDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
        ...
        submenu: [
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Venidium-Network/venidium-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Venidium-Network/venidium-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Venidium-Network/venidium-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://discord.gg/yNy4HdXP87');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/Venidiumio');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Venidium Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Venidium' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Venidium Blockchain' }),
            click: () => {
              openAbout();
            },
          },

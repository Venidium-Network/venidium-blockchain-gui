...
global.daemon_rpc_ws = `wss://${self_hostname}:52700`;
...
function loadConfig(net) {
  try {
    // check if VENIDIUM_ROOT is set. it overrides 'net'
    const config_root_dir =
      'VENIDIUM_ROOT' in process.env
        ? process.env.VENIDIUM_ROOT
        : path.join(os.homedir(), '.venidium', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 52700); // jshint ignore:line

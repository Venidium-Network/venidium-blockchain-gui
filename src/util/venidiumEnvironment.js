...
const PY_FOLDER = '../venidium/daemon';
...
const getVenidiumVersion = () => {
  let version = null;
  const exePath = getExecutablePath('venidium');
  // first see if we can get a venidium exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or venidium is on the path
    try {
      version = child_process
